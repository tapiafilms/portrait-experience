...
export default function App() {
  ...
  // Al terminar la captura, lanzar transform automáticamente
  const handleCaptureDone = useCallback(async (data, name, guestId) => {
    ...
    try {
      const result = await transformCapture({
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', overflow: 'hidden', background: '#0a0a0a' }}>
      ...
      {/* Pantalla de espera mientras la IA procesa */}
      {screen === 'transforming' && (
...
const ts = {
  ...
  logoTitle: {
    height: '42px', objectFit: 'contain', zIndex: 2,
  },
  content: {
    display: 'flex', flexDirection: 'column',
    alignItems: 'center', gap: '28px', zIndex: 2,
  },
  orbWrapper: {
  ...
  orb: {
    ...
    boxShadow: '0 0 40px rgba(96,165,250,0.7), 0 0 80px rgba(59,130,246,0.4)',
  ...
  label: {
    fontSize: 11, letterSpacing: 6, color: 'rgba(147,197,253,0.9)',
